module SimpleTokenAuthentication
  module ActsAsTokenAuthenticationHandlerMethods
    ...
    def authenticate_entity_from_token!(entity_class)
      ...
      params_uuid_name = "#{entity_class.name.singularize.underscore}_uuid".to_sym
      if token = params[params_token_name].blank? && request.headers[header_token_name(entity_class)]
        params[params_token_name] = token
      end
      if uuid = params[params_uuid_name].blank? && request.headers[header_uuid_name(entity_class)]
        params[params_uuid_name] = uuid
      end

      uuid = params[params_uuid_name].presence
      # See https://github.com/ryanb/cancan/blob/1.6.10/lib/cancan/controller_resource.rb#L108-L111
      entity = nil
      if entity_class.respond_to? "find_by"
        entity = uuid && entity_class.find_by(uuid: uuid)
      elsif entity_class.respond_to? "find_by_uuid"
        entity = uuid && entity_class.find_by_uuid(uuid)
      end
    ...
    # Private: Return the name of the header to watch for the uuid param
    def header_uuid_name(entity_class)
      if SimpleTokenAuthentication.header_names["#{entity_class.name.singularize.underscore}".to_sym].presence
        SimpleTokenAuthentication.header_names["#{entity_class.name.singularize.underscore}".to_sym][:uuid]
      else
        "X-#{entity_class.name.singularize.camelize}-UUID"
      end
    end
  end
